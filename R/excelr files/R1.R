...
#we call string data type as character data type in R
...
#vector- assigning multiple values to single variable
...
# logical vector
#difference between vector & list 
#vector-displays data type based on priority of individual elements' datatype
#list- always displays data type as 'List'
...
h[[3]][1]
h[1:3]
g <- c(23, 21.3, c(2,5,3), "hello",sin)#inbuilt function-> predefined already, no need of calling them to use it
g
class(g) #Due to inbuilt function 'sin' present in vector,each element gets separated as independent index portion. hence, it is converted into list.
View(g)
...
m$height #horizontal representation
m$gender
m$gender[3]
m[2]
m[2,] #to have only 2nd row
m[[2]] #horizontal representation
m[[2]][2]
...
#ctrl+shift+c -> to comment multiple lines in R
...
?ls #to search the pattern->ls, ?->help
ls()
a <- 3
...
v1 <- c(3,1,TRUE,2+3i) #values get converted to highest priority data types in vector : 3+0i 1+0i 1+0i 2+3i
v2 <<- c(3,1,TRUE,2+3i)
v3 = c(3,1,TRUE,2+3i)
v1
v2
