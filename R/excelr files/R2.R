...
#ifelse(contition , true , ifelse(condition , true , ifelse(condition , true , otherwise value)))
# Loops in R
# While loop
# Syntax
...
i <- 1 #until unless condition condition gets failed, loop continues to execute
while (i < 6) {
  print(i)
   i = i+1
}
...
var=readline(); #to get input
var=as.integer(var); #to convert the input into integer
print(var)
...
library(modeest)
mlv(mySamples, method = "mfv") #mlv-most likely value, mfv-most frequency value
